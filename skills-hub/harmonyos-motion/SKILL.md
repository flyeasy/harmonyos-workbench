...
常用映射：按下即变是**按下反馈**；离手急刹车是**速度继承缺失/速度断层**（先排查长帧）；列表到头硬截是**边界阻尼/回稳缺失**；轻拉就刷新是**刷新阈值/armed 状态缺失**；刷新中重复触发是**刷新防重入缺失**；对象随手是**跟手**，新输入接管是**可打断动画**；同一对象跨页面连续是**共享元素/容器转场**；短暂按钮点不到是**可见期**或旧几何；动画早于业务成功是**虚假完成**；旧回调覆盖新意图是**响应乱序/陈旧计时器**；IME 改坏远端内容是**局部/远端几何耦合**。
